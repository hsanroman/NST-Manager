...
def authPage() {
	...
	if(!oauthTokenProvided && atomicState?.accessToken) {
		...
		log.info "AuthPage Render Time: (${(now()-execTime)}) milliseconds)"
		return mainPage()
	}
}
...
def mainPage() {
	...
	return dynamicPage(name: "mainPage", title: "", nextPage: (!setupComplete ? "reviewSetupPage" : null), install: setupComplete, uninstall: false) {
		...
		incMainLoadCnt()
		log.info "MainPage Render Time: (${(now()-execTime)} milliseconds)"
	}
}
...
def devicesPage() {
	...
	if (settings?.structures) {
		...
		incDevLocLoadCnt()
		log.info "DevicesPage Render Time: (${(now()-execTime)} milliseconds)"
	}
}
...
def automationsPage() {
	...
	return dynamicPage(name: "automationsPage", title: "Installed Automations", nextPage: !parent ? "" : "automationsPage", install: false) {
		...
		incAutoLoadCnt()
		log.info "AutomationsPage Render Time: (${(now()-execTime)} milliseconds)"
	}
}
...
def automationSchedulePage() {
	...
	dynamicPage(name: "automationSchedulePage", title: "View Schedule Data..", uninstall: false) {
		...
		log.info "AutomationSchedulePage Render Time: (${(now()-execTime)} milliseconds)"
	}
}
...
def automationStatisticsPage() {
	...
	dynamicPage(name: "automationStatisticsPage", title: "Installed Automations Stats\n(Auto-Refresh Every 20 sec.)", refreshInterval: 20, uninstall: false) {
		...
		log.info "AutomationStatisticsPage Render Time: (${(now()-execTime)} milliseconds)"
	}
}
...
def notifPrefPage() {
	...
	dynamicPage(name: "notifPrefPage", install: false) {
		...
		incNotifPrefLoadCnt()
		log.info "NotifPrefPage Render Time: (${(now()-execTime)} milliseconds)"
	}
}
...
def devNamePage() {
	...
	dynamicPage(name: "devNamePage", title: pageLbl, nextPage: "", install: false) {
		...
		log.info "DevNamePage Render Time: (${(now()-execTime)} milliseconds)"
	}
}
...
def devPrefPage() {
	...
	dynamicPage(name: "devPrefPage", title: "Device Preferences", uninstall: false) {
		...
		incDevCustLoadCnt()
		log.info "DevPrefPage Render Time: (${(now()-execTime)} milliseconds)"
	}
}
...
def debugPrefPage() {
	...
	dynamicPage(name: "debugPrefPage", install: false) {
		...
		atomicState.needChildUpd = true
		incLogPrefLoadCnt()
		log.info "DebugPrefPage Render Time: (${(now()-execTime)} milliseconds)"
	}
}
...
def getInstAutoTypesDesc() {
	...
	str += (sch?.size()) ? "\n• Thermostat (${sch?.size()})" : ""
	str += (disItems?.size() > 0) ? "\n\nDisabled Automations: (${disItems?.size()})" : ""
	// disItems?.each { di ->
	// 	str += (disItems?.size() > 0) ? "\n• $di" : ""
	// }
	return (str != "") ? str : null
